...
export async function GET() {
  try {
    ...
    const wrapperRepositories = (repositories || []).filter((repo: { id: string; name?: string | null }) =>
    ...
    const items = await Promise.all(
      wrapperRepositories.map((repo: { id: string; name?: string | null }) =>
...
export async function POST(req: Request) {
  ...
  const inferredName =
    requestedName ?? githubRepoName?.split("/").pop()?.trim() ?? "App Project";
  ...
  try {
    ...
    // Safe Local Fallback: when cloud microVM key is not present, create a local session
    const localRepoId = `local-${randomUUID().slice(0, 8)}`;
    const conversationId = randomUUID();
    const localMetadata: RepoMetadata = {
      version: 2,
      sourceRepoId: localRepoId,
      name: inferredName,
      vm: {
        vmId: `local-vm-${localRepoId}`,
      ...
      conversations: [
        {
          id: conversationId,
          title: requestedConversationTitle || inferredName,
