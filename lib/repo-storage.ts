...
// In-Memory Storage Cache for Local sessions & offline resilience
const memoryStore = new Map<string, Map<string, string>>();

const getMemoryFile = (repoId: string, path: string): string | null => {
  return memoryStore.get(repoId)?.get(path) ?? null;
};

const setMemoryFile = (repoId: string, path: string, content: string) => {
  let repoMap = memoryStore.get(repoId);
  if (!repoMap) {
    repoMap = new Map<string, string>();
    memoryStore.set(repoId, repoMap);
  }
  repoMap.set(path, content);
};
...
const readJsonFile = async <T>(
  ...
  const localVal = getMemoryFile(repoId, path);
  if (localVal) {
    try {
...
const writeCommit = async (
  ...
  for (const f of files) {
    setMemoryFile(repoId, f.path, f.content);
  }
  ...
  try {
    ...
    // Graceful fallback for local or offline sessions
  }
};
